...
(function($){
    function init(plot){
        ...
        function calculPixel2XWidthConvert(plot, serie){
            ...
            var minMaxValues = getXabsMinMaxValues(serie);
            var XAbscisseWidth = minMaxValues[1] - minMaxValues[0];
            pixelInXWidthEquivalent = XAbscisseWidth / gridWidth;
        }

        function getXabsMinMaxValues(serie){
            var minMaxValues = new Array();
            minMaxValues[0] = serie.data[0][0];
            minMaxValues[1] = serie.data[serie.data.length - 1][0];
